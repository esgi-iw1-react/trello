...
const CardPreview = ({ card, list, index }) => {
  return <>
        ...
        <Link to={"app/card/"+card._id}>
          ...
          >
            ...
            <div className="flex mt-2 justify-end">
              {
                card.users && card.users.map((user, index) => { return <UserAvatar key={index} username={user.username.charAt(0).toUpperCase()}/>   })
              }
            </div>
          </div>
        </Link>
    ...
    <Route path={"/app/card/"+card._id} exact render={(props) => <Card {...props} card={card} list={list} />}/>
  </>
